...
date_time=$(date +%s)
ssh_command="USER=svc.buildpacks nimbus deploy ovf --queue --wait-scheduler buildpacks-private-concourse-worker-$date_time  NIMBUS_OVF_BASE/svc.buildpacks/buildpacks-concourse-worker/buildpacks-concourse-worker-ovf.ovf"
ssh ${username}@nimbus-gateway bash -c "'$ssh_command'"

# get IP address from output
echo "Resolving worker IP Address..."
ip_json=$(ssh ${username}@nimbus-gateway bash -c "'USER=svc.buildpacks nimbus-ctl --outputFormat=json ip svc.buildpacks-buildpacks-private-concourse-worker-$date_time | grep json_info'")
ip=$(echo ${ip_json} | jq -r '.json_info[]."svc.buildpacks-buildpacks-private-concourse-worker-'$date_time'"')


# ssh onto the machine and add the worker private key
echo "Accessing Concourse Credhub..."
source ~/workspace/bp-envs/scripts/login_director_private
# TODO Do something reasonable if that script isnt there

echo "Setting up credentials for worker..."
credhub get -n /bosh-buildpacks-private/concourse/nimbus_worker_private_key -q > /tmp/deleteme-private-key
...
cat concourse.service.private | sed -e "s/datetime/$date_time/" > /tmp/deleteme-concourse-config
...
fly -t buildpacks-private workers | grep ${date_time}
}

function usage() {
  cat <<-USAGE
deploy-private-nimbus-worker.sh --username <nimbus gateway username>

Deploys a nimbus vsphere worker to the private concourse.
