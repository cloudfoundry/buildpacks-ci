  ...
  describe '#run!' do
    ...
    context('dotnet') do
      ...
      before do
        ...
        allow(subject).to receive(:get_framework_version).and_return '1.0.2'
      end
      ...
      it "updates the dotnet buildpack manifest dependency with the specified version" do
        ...
        dependency_in_manifest = manifest["dependencies"].find{|dep| dep["name"] == dependency && dep["version"] == '1.0.0-preview1-002702'}
        expect(dependency_in_manifest).to eq(nil)
      ...
      it "updates the dotnet-versions.yml specified version and its framework version" do
        ...
        dependency_in_versions = versions.find{|ver| ver["dotnet"] == '1.0.0-preview1-002702'}
        expect(dependency_in_versions).to eq(nil)

        dependency_in_versions = versions.find{|ver| ver["dotnet"] == '1.0.0'}
        expect(dependency_in_versions['dotnet']).to eq('1.0.0')
        expect(dependency_in_versions['framework']).to eq('1.0.2')
      end

      it 'records which versions were removed' do
        subject.run!
        expect(subject.removed_versions).to eq(['1.0.0-preview1-002702'])
      end
    end
  end
