...
describe SlackClient do
  ...
  describe '#initialize' do
    context 'the slack_webhook is nil' do
      subject { SlackClient.new(nil, channel, username) }
    ...
    context 'the channel is nil' do
      subject { SlackClient.new(slack_webhook, nil, username) }
    ...
    context 'the slack_webhook is empty' do
      subject { SlackClient.new('', channel, username) }
    ...
    context 'the channel is empty' do
      subject { SlackClient.new(slack_webhook, '', username) }
    ...
    context 'the slack_webhook is not a string' do
      subject { SlackClient.new(12345, channel, username) }
    ...
    context 'the channel is not a string' do
      subject { SlackClient.new(slack_webhook, 12345, username) }
    ...
    context 'the username is nil' do
      subject { SlackClient.new(slack_webhook, channel, nil) }
    ...
    context 'the username is empty' do
      subject { SlackClient.new(slack_webhook, channel, '') }
  ...
  describe '#post_to_slack' do
    ...
    describe 'input checking' do
      ...
      context 'the POST request is successful' do
        subject { SlackClient.new(slack_webhook, channel, username) }
