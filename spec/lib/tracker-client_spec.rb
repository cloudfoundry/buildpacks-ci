...
describe TrackerClient do
  ...
  let (:requester_id) { 1234567 }

  describe '#initialize' do
    context 'the api key is nil' do
      subject { TrackerClient.new(nil, project_id, requester_id) }
    ...
    context 'the project id is nil' do
      subject { TrackerClient.new(api_key, nil, requester_id) }
    ...
    context 'the api key is empty' do
      subject { TrackerClient.new('', project_id, requester_id) }
    ...
    context 'the project id is empty' do
      subject { TrackerClient.new(api_key, '', requester_id) }
    ...
    context 'the api key is not a string' do
      subject { TrackerClient.new(12345, project_id, requester_id) }
    ...
    context 'the project id is not a string' do
      subject { TrackerClient.new(api_key, 12345, requester_id) }
    ...
    context 'the requester id is nil' do
      subject { TrackerClient.new(api_key, project_id, nil) }
    ...
    context 'the requester id is not a number' do
      subject { TrackerClient.new(api_key, project_id, '') }
  ...
  describe '#post_to_tracker' do
    ...
    describe 'input checking' do
      ...
      context 'the POST request is successful' do
        subject { TrackerClient.new(api_key, project_id, requester_id) }

        before do
          stub_request(:post, tracker_uri).
            with(:body => "{\"name\":\"OH NOOOO\",\"description\":\"OH NOOOOOOOOOOOOOOOOOO\",\"requested_by_id\":1234567}",
        ...
        it 'receives the 200 status code' do
          response = subject.post_to_tracker name, description
      ...
      context 'the story name is empty' do
        ...
        it 'raises an exception without posting to Tracker' do
          expect{ subject.post_to_tracker '', 'WHAT HAVE YOU DOONNNNNNNNNNE' }.to raise_error(RuntimeError)
      ...
      context 'the story name is nil' do
        ...
        it 'raises an exception without posting to Tracker' do
          expect{ subject.post_to_tracker nil, 'WHAT HAVE YOU DOONNNNNNNNNNE' }.to raise_error(RuntimeError)
          expect(WebMock).not_to have_requested(:post, tracker_uri)
        end
      end
      ...
      context 'the story description is empty' do
        ...
        it 'raises an exception without posting to Tracker' do
          expect{ subject.post_to_tracker 'a', '' }.to raise_error(RuntimeError)
          expect(WebMock).not_to have_requested(:post, tracker_uri)
        end
      end
      ...
      context 'the story description is nil' do
        ...
        it 'raises an exception without posting to Tracker' do
          expect{ subject.post_to_tracker 'a', nil }.to raise_error(RuntimeError)
          expect(WebMock).not_to have_requested(:post, tracker_uri)
        end
      end
    end
