class CNBDependencies
  ...
  def switch
    ...
    if @matching_deps.map { |d| d['version'] }.include?(@dep['version'])
      out = ((@dependencies.reject { |d| same_dependency_line?(d['stacks'], d['version'], d['id']) && d['version'] == @dep['version'] }) + [@dep])
  ...
  def dep_includes_at_least_these_stacks?(manifest_stacks)
    (manifest_stacks - @dep['stacks']).empty?
  end

  def same_dependency_line?(stacks, version, dep_id)
    return false if dep_id != @dep['id']
