...
class FinalizeSecurityNoticeStories
  def initialize(tracker_client, new_stack_version, davos_client)
  ...
  def run
    ...
    fixed_affected_stories.each do |story|
      ...
      @davos_client.change(davos_id(story), status: 'fixed', affected_versions: '', fixed_versions: @new_stack_version)
    end
  end
