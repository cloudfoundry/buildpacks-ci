      ...
      Dir.mktmpdir do |dir|
        Dir.chdir(dir) do
          ...
          when 'cflinuxfs5'
            ...
            raise "Unsupported stack for R build: #{stack}"
          end
...
class DependencyBuild
  ...
  class Utils
    ...
    def self.setup_gcc
      ...
      when 'cflinuxfs5'
        Runner.run('apt', 'install', '-y', 'gcc-14', 'g++-14')
        Runner.run('update-alternatives', '--install', '/usr/bin/gcc', 'gcc', '/usr/bin/gcc-14', '60', '--slave', '/usr/bin/g++', 'g++', '/usr/bin/g++-14')
      else
        # Default to latest available
        Runner.run('apt', 'install', '-y', 'gcc', 'g++')
      end
    end
