...
previous_version = File.read('previous-cflinuxfs3-release/version').strip

if ENV.fetch('STACK') == 'cflinuxfs3'
  old_receipt_uri = "https://raw.githubusercontent.com/cloudfoundry/cflinuxfs3/#{previous_version}/cflinuxfs3/cflinuxfs3_receipt"
  receipt_diff_file = File.join('receipt-diffs', 'cflinuxfs3-diff')
else
  raise "Unsupported stack: #{ENV.fetch('STACK')}"
end

new_receipt_file = Dir["receipt-artifacts/cflinuxfs3_receipt*"].first
