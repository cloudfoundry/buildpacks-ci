...
class ExtractDotnetFramework
  ...
  def retar
    ...
    raise "Currently, we only expect one dotnet framework to be bundled with each sdk." if dotnet_framework_versions.size != 1

    @dotnet_framework_version = dotnet_framework_versions.first

    dotnet_framework_artifact_dir = File.expand_path(File.join(@buildpacks_ci_dir, '..', 'binary-builder-artifacts'))

    @dotnet_framework_tar = File.join(dotnet_framework_artifact_dir, "dotnet-framework.#{@dotnet_framework_version}.linux-amd64.tar.gz")

    Dir.chdir(@dotnet_sdk_dir) do
      raise "Tarring the dotnet framework failed" unless system("tar zcf #{@dotnet_framework_tar} shared host *.txt")
    end
  end
  ...
  def write_yaml
    ...
    framework_built['dotnet-framework'].push({
      'version' => @dotnet_framework_version,
    ...
    md5sum = Digest::MD5.file(@dotnet_framework_tar).hexdigest
    shasum = Digest::SHA256.file(@dotnet_framework_tar).hexdigest

    git_msg = "Build dotnet-framework - #{@dotnet_framework_version}\n\n"

    git_yaml = {
      'filename' => File.basename(@dotnet_framework_tar),
      'version' => @dotnet_framework_version,
      'md5' => md5sum,
      'sha256' => shasum,
