...
func TestFactoryIntegration(t *testing.T) {
	...
	RunSpecs(t, "Factory Integration Test Suite")
}

var _ = BeforeSuite(func() {
	// Integration tests require a GitHub token to avoid rate limiting
	// and to test authenticated API access
	if os.Getenv("GITHUB_TOKEN") == "" {
		Skip("GITHUB_TOKEN environment variable not set. GitHub integration tests require authentication.\n" +
			"Create a token at https://github.com/settings/tokens and export GITHUB_TOKEN=\"your_token\"")
	}
})
...
var _ = Describe("Factory", func() {
	Describe("Check", func() {
		Context("with valid source types", func() {
			It("routes github_releases to GithubReleasesWatcher", func() {
			...
			It("routes github_tags to GithubTagsWatcher", func() {
				source := factory.Source{
					Type:     "github_tags",
					Repo:     "ruby/ruby",
					TagRegex: `^v[\d_]+$`,
		...
		Context("with github_releases options", func() {
			It("applies extension filter", func() {
				source := factory.Source{
					Type:      "github_releases",
					Repo:      "cloudfoundry/hwc",
					Extension: ".exe",
