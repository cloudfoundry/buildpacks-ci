...
RUN cd /usr/local \
  && curl -L https://dl.google.com/go/go1.17.linux-amd64.tar.gz -o go.tar.gz \
  && [ 6bf89fc4f5ad763871cf7eac80a2d594492de7a818303283f1366a7f6a30372d = $(shasum -a 256 go.tar.gz | cut -d' ' -f1) ] \
  && tar xf go.tar.gz \
  && rm go.tar.gz
